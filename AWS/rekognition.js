...
module.exports = class Rekognition {
  ...
  async uploadToS3(imagePaths, folder) {
    ...
    else return await this.s3.upload(imagePaths, folder);
  }
  ...
  /**
   * Detects instances of real-world customs labels within an image
   *
   * @param {Object|Buffer} image
   * @param {string} arn
   * @param {string} threshold
   */
  async detectCustomLabels(image, arn, threshold = 50) {
    const params = {
      Image: this.getImageParams(image),
      MinConfidence: threshold,
    ...
    return await this.doCall("detectCustomLabels", params);
  }
